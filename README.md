...
&nbsp;
**[Quick Start](#quick-start)** · **[Features](#features)** · **[DNS Requirement](#dns-requirement)** · **[Get PRO →](https://noctisapi.com)**
...
| Geo enrichment via configurable resolver | ✓ |
...
Before running the production installer, create an `A` record for the public domain you will use and point it to the server's public IP address.
...
This must be in place before deployment because `HP_PUBLIC_HOST`, Traefik routing, and the Let's Encrypt HTTP challenge all depend on the domain resolving to the server.
...
| `HP_ASN_RESOLVER_URL` | Geo resolver URL, defaults to `https://ipwho.is/{ip}` |
